# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/molecule.py
...
from formula_design.mol import rkutil
from formula_design.mol.conformer import (Conformer, is_charge_key,
                                          is_energy_key, is_force_key,
                                          is_smiles_key, is_topo_params_key,
                                          prefix_prop_key,
                                          write_conformers_to_extxyz,
                                          xyz_properties_parser)
from formula_design.utils.definitions import TopoParams
...
class Molecule:
    ...
    def __init__(self,
        ...
        self._aromaticity = kwargs.get(
            'aromaticity',
            'rdkit')  # default to rdkit, unless you manually specify mdl
        assert self._aromaticity in [
            'rdkit', 'mdl'
        ], f'aromaticity must be either rdkit or mdl, got {self._aromaticity}'

        if isinstance(molecule, Chem.Mol):
            # number of conformers = number of conformers included in molecule
            self._from_rkmol(molecule,
                             keep_conformers=keep_conformers,
                             keep_mol_prop=keep_mol_prop,
                             **kwargs)
        else:
            ...
            if fmt == 'xyz':
                ...
                if fmt == 'sdf':
                    ...
                    self._from_sdf(molecule,
                                   keep_conformers=keep_conformers,
                                   **kwargs)

                elif fmt == 'smiles':
                    self._from_smiles(molecule, kwargs.pop('nconfs', 0),
                                      **kwargs)
                elif fmt == "mapped_smiles":
                    self._from_mapped_smiles(molecule, kwargs.pop('nconfs', 0),
                                             **kwargs)
    ...
    def _sanitize(self):
        ...
        rkmol = rkutil.sanitize_rkmol(self.rkmol,
                                      aromaticity=self._aromaticity)
    ...
    def _finish(self, **kwargs):
        ...
        # sanitize again
        self.rkmol = rkutil.sanitize_rkmol(self.rkmol,
                                           aromaticity=self._aromaticity)
        ...
        if self.name is None or len(self.name) == 0:
            ...
            self.name = rkutil.get_mol_formula(
                self.rkmol) + '_' + m.hexdigest()
        return

    def get_bonds(self) -> List[Tuple]:
        '''get bond list (i,j) and i<j'''
        bonds = [
            tuple(sorted([bond.GetBeginAtomIdx(),
                          bond.GetEndAtomIdx()]))
    ...
    def get_smiles(self,
        ...
        return rkutil.get_smiles(self.rkmol,
                                 isomeric=isomeric,
                                 kekulize=kekulize)
    ...
    def merge(self, other: "Molecule", check_chiral: bool = True):
        ...
        if not self.get_mapped_smiles(
                isomeric=check_chiral) == other.get_mapped_smiles(
                    isomeric=check_chiral):
    ...
    def _parse_conformers(self):
        ...
        for conf in rkconfs:
            coords = np.vstack(
                [conf.GetAtomPosition(idx) for idx in range(natoms)])
    ...
    def _from_rkmol(self,
        ...
        self._atomic_symbols = [
            atom.GetSymbol() for atom in self.rkmol.GetAtoms()
        ]
        self._atomic_masses = [
            atom.GetMass() for atom in self.rkmol.GetAtoms()
        ]
        self._formal_charges = [
            atom.GetFormalCharge() for atom in self.rkmol.GetAtoms()
        ]
        ...
        if nconfs > 0:
            self.rkmol, _, _ = rkutil.generate_confs(self.rkmol,
                                                     nconfs=nconfs,
                                                     **kwargs)
    ...
    def _from_xyz(self, filename: str, mapped_smiles: str = None, **kwargs):
        ...
        atoms_list = aseio.read(filename,
                                index=':',
                                format="extxyz",
                                properties_parser=xyz_properties_parser)
        ...
        if mapped_smiles is None:
            raise MappedSmilesMissingException(
                "'mapped_smiles' missed in xyz file.")

        # this conformer is only used to infer stereochemistry
        rkmol = rkutil.get_mol_from_smiles(mapped_smiles)
        rkmol = rkutil.renumber_atoms_with_atom_map_num(rkmol)
        rkmol = rkutil.append_conformers_to_mol(rkmol,
                                                [atoms_list[0].positions])
        ...
        for i in range(1, len(atoms_list)):
            common_info = common_info & set([
                (k, v) for k, v in atoms_list[i].info.items()
            ])  # find identical (k,v) pairs

        for k, v in common_info:
            if is_charge_key(k) or is_energy_key(k) or is_force_key(
                    k) or is_smiles_key(k):
                continue
        ...
        if keep_conformers:
            conf_list = [
                Conformer.from_ase_atoms(atoms) for atoms in atoms_list
            ]
    ...
    def to_rkmol(self, conf_id: int = None) -> Chem.Mol:
        ...
        rkmol = rkutil.append_conformers_to_mol(
            rkmol, [conf.coords for conf in conf_list])
    ...
    def to_xyz(self, filename: str, conf_id=None, append=False, confkeys=None):
        ...
        write_conformers_to_extxyz(conf_list,
                                   filename,
                                   append=append,
                                   confkeys=confkeys)
        for i, conf in enumerate(conf_list):
    ...
    def copy(self, keep_conformers=True):
        ...
        if keep_conformers and self.nconfs > 0:
            rkmol = rkutil.append_conformers_to_mol(
                rkmol, [self.conformers[0].coords])
    ...
    def append_conformers(self, conformers: Union[List[Conformer], Conformer]):
        ...
        for conf in conformers:
            ...
            if first_conf is None:
                ...
                assert set(first_conf.confdata.keys()) == set(
                    conf.confdata.keys())
    ...
    def _get_all_confdata(self) -> dict:
        ...
        return {
            k: [conf.confdata[k] for conf in self.conformers]
            for k in conf.confdata
        }
    ...
    def get_confdata(self, prop: str):
        ...
        if not prop in confdata:
            raise KeyError(
                "conformer property {} is not in confdata".format(prop))
    ...
    def set_partial_charges(self,
                            ...
                            partial_charges_key: str = 'partial_charges'):
        assert is_charge_key(
            partial_charges_key
        ), 'partial charges key name must end with _charge or _charges'
        assert self.nconfs > 0

        partial_charges = np.asarray(partial_charges)

        if partial_charges.ndim == 1:
            assert partial_charges.shape[
                0] == self.natoms, 'partial_charges does not match natoms'
            for conf in self.conformers:
                conf.confdata[partial_charges_key] = np.asarray(
                    partial_charges)
        elif partial_charges.ndim == 2:
            assert partial_charges.shape[
                0] == self.nconfs, 'partial_charges does not match nconfs'
            assert partial_charges.shape[
                1] == self.natoms, 'partial_charges does not match natoms'
        ...
        try:
            ...
            if skip_error:
                ...
                raise AssertionError(
                    'parsing molecule from sdf file failed') from e

    return result


def read_molecules_from_xyz(xyz_file: str,
                            check_chiral: bool = True) -> Dict[str, Molecule]:
    atoms_list = aseio.read(xyz_file,
                            index=':',
                            format="extxyz",
                            properties_parser=xyz_properties_parser)
...
def assert_good_molecule(mol: Molecule) -> None:
    ...
    assert set(mol.atomic_numbers).issubset(
        good_elements), f'bad elements found: {mol.atomic_numbers}'
    ...
    for idx, num in enumerate(mol.atomic_numbers):
        if num in [9, 17, 35, 53]:
            assert connectivity[
                idx] == 1, f'bad connectivity: idx {idx} atomic number {num} is X{connectivity[idx]}'
