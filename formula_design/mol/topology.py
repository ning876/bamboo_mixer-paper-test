# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/topology.py
...
class Topology:
                ...
                for i, a1 in enumerate(neighbors[:-2]):
                    for j, a2 in enumerate(neighbors[i + 1:-1]):
                        for a3 in neighbors[i + j + 2:]:
                            atomsets.add(
                                rkutil.sorted_atomids((atom, a1, a2, a3),
                                                      is_improper=True))
    ...
    def _cal_nonbonded(self) -> Tuple[List[Tuple], List[Tuple]]:
        ...
        nonbonded14 = set([tuple(sorted([p[0], p[3]]))
                           for p in self._propers]) - nonbonded12 - nonbonded13
        nonbondedall = set([
            tuple(sorted([self.atoms[i], self.atoms[j]]))
            for i in range(self.natoms - 1) for j in range(i + 1, self.natoms)
        ])
        nonbondedall = nonbondedall - nonbonded14 - nonbonded13 - nonbonded12
        nonbondedall, nonbonded14 = sorted(list(nonbondedall)), sorted(
            list(nonbonded14))
    ...
    def nonbonded14_pairs(self) -> List[Tuple[int]]:
        if self._nonbonded14 is None:
            nonbonded12 = set(
                [tuple(sorted([p[0], p[1]])) for p in self._bonds])
            nonbonded13 = set(
                [tuple(sorted([p[0], p[2]])) for p in self._angles])
            nonbonded14 = set(
                [tuple(sorted([p[0], p[3]]))
                 for p in self._propers]) - nonbonded12 - nonbonded13
    ...
    def rings(self) -> List[List[int]]:
        if self._rings is None:
            self._rings = self._cal_rings(self.graph,
                                          max_ring_size=self._max_include_ring)
        return self._rings
    ...
    def get_nonring_dihedral_rotate_atoms(self, *indices:
        ...
        while to_scan_list:
            ...
            new_neighbor_set = set(
                self.adj_list[idx]) - set(scanned_list) - set(to_scan_list)
