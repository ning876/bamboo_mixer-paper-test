# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/conformer.py
...
from formula_design.utils import simple_unit as unit
...
class Conformer:
    ...
    def coords(self, coords: np.array):
        assert isinstance(coords,
                          np.ndarray) and coords.shape == (self.natoms, 3)
        self.confdata["coords"] = coords
    ...
    def from_xyz(cls, xyz_file: str):
        ...
        assert len(
            atoms_list
        ) == 1, "Unable to load multi xyz confs to one Conformer object"
    ...
    def to_ase_atoms(self, confkeys=None):
        ...
        for key in confkeys:
            if is_force_key(key):
                atoms.arrays[key] = unit.kcal_mol_A_to_eV_A(
                    np.array(self.confdata[key]))
    ...
    def copy(self):
        return Conformer(self.coords.copy(),
                         copy.copy(self.symbols),
                         confdata=copy.deepcopy(self.confdata))
