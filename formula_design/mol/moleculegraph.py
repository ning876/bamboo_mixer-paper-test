# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/moleculegraph.py
...
from formula_design.mol.rkutil.match_and_map import find_mapped_smarts_matches
from formula_design.mol.topology import Topology
...
class Atom:
    ...
    def __init__(self, atom: Chem.Atom = None, idx: int = None):
        ...
        if atom is None:
            ...
            self.hybrdization = hyb_map.get(atom.GetHybridization(),
                                            Hybridization.OTHER)
...
class MoleculeGraph:
    ...
    def __init__(self,
        ...
        if mol is not None:
            ...
            self.rkmol = rkutil.sanitize_rkmol(mol.get_rkmol(),
    ...
    def _create_from_rkmol(self):
        ...
        self.topo = Topology(bonds=bonds,
                             bond_ids=bond_ids,
                             max_include_ring=self.max_include_ring)
    ...
    def update_graph_info(self, rings: list[list[int]] = None):
        ...
        for ring in rings:
            for pair in zip(ring, ring[1:] + [ring[0]]):
                bond = self.get_connect_bond(
                    *[self.get_atom(idx) for idx in pair])
                bond.in_ring = True
        # set atom ring info
        for atom in self.get_atoms():
            atom.connectivity = len(self.get_neighbors(atom))
            atom.ring_connectivity = len([
                bond for bond in self.get_neighbor_bonds(atom) if bond.in_ring
            ])
    ...
    def get_bond_with_atoms(self) -> list[tuple[Bond, Atom, Atom]]:
        return [(self._bond_by_idx[data['idx']], self._atom_by_idx[a1],
                 self._atom_by_idx[a2])
    ...
    def get_neighbor_bonds(self, atom: Atom) -> list[Bond]:
        return [
            self.get_connect_bond(atom, nei)
            for nei in self.get_neighbors(atom)
        ]
    ...
    def get_impropers(self) -> list[tuple[int]]:
        ...
        for atom in self.get_atoms():
            if atom.atomic_number in [6, 7]:
                ...
                if len(nei) == 3:
                    atomsets.add(
                        rkutil.sorted_atomids((idx, nei[0], nei[1], nei[2]),
                                              is_improper=True))
    ...
    def get_nonring_dihedral_rotate_atoms(self, *indices:
                                          list[int]) -> list[int]:
    ...
    def get_intra_topo(self) -> dict[str, list[tuple[int]]]:
        ...
        return {
            'Bond': bonds,
            'Angle': angles,
            'ProperTorsion': propers,
            'ImproperTorsion': impropers
        }
    ...
    def get_linear_propers(self) -> list[tuple[int]]:
        ...
        rkmol = self.get_rkmol()
        linear_patternC = Chem.MolFromSmarts(
            '[*:1]~[#6X2;!r5;!r6:2]~[*:3]~[*:4]')
        linear_patternN = Chem.MolFromSmarts(
            '[!$([#7H1]):1]~[#7X2+:2]~[!$([#7H1]):3]~[*:4]')
    ...
    def _calc_priority(self, i: int, j: int, k: int, l: int,
                       in_ring: bool) -> tuple:
    ...
    def get_proper_from_bonds(
            self, bonds: list[tuple[int, int]]) -> list[tuple[int]]:
...
class MutableMoleculeGraph(MoleculeGraph):
    ...
    def to_rkmol(self, set_coord=False):
        ...
        for bond, a1, a2 in self.get_bond_with_atoms():
            ...
            bond_type = rkutil.num_bondorder.get(bond.order,
    ...
    def update_topo(self):
        ...
        for bond_idx in self._bond_by_idx.keys():
            ...
            bonds.append((self._bond_by_idx[bond_idx].begin_idx,
                          self._bond_by_idx[bond_idx].end_idx))
