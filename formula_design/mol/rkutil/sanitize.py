# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/rkutil/sanitize.py
...
# check rdkit version, allow version newer than 2022.9
assert version.parse(rdkit.rdBase.rdkitVersion) >= version.parse(
    "2022.9.0"), f"RDKit version too low: {rdkit.rdBase.rdkitVersion}"
...
def get_mol_from_smiles(smiles: str,
    ...
    if 'sanitize' in kwargs:
        logger.warning(
            'sanitize option currently has no effect in this function')
...
normalization_transforms = {
    ...
    'Deprotonated sulfonamide':
    '[N:1]=[SX4:2]([O-:3])=[O:4]>>[N-:1]-[SX4:2](=[O-0:3])=[O:4]',
    ...
    'Odd pyridine/pyridazine oxide structure C':
    '[C;-;D2,D3:1]-[N+2;D3:2]-[O-;D1:3]>>[C;-0:1]=[N+1:2]-[O-:3]',
    'Odd pyridine/pyridazine oxide structure N':
    '[N;-;D2,D3:1]-[N+2;D3:2]-[O-;D1:3]>>[N;-0:1]=[N+1:2]-[O-:3]',
...
def normalize_rkmol(rkmol: Chem.Mol) -> Chem.Mol:
    ...
    for name, reaction in normalization_transforms.items():
        ...
        if count > 0:
            logger.info(
                f'{name} transform {reaction} has been applied {count} times.')
...
def sanitize_rkmol(rkmol: Chem.Mol,
    ...
    try:
        ...
        if allow_advanced_skip_kekulize:
            logger.warning(
                'fail to sanitize, retry sanitizing without kekulizing')
    ...
    try:
        ...
        if aromaticity.lower() == 'mdl':
            ...
            Chem.SetAromaticity(_rkmol,
                                Chem.AromaticityModel.AROMATICITY_RDKIT)
        elif aromaticity.lower() == 'simple':
            Chem.SetAromaticity(_rkmol,
                                Chem.AromaticityModel.AROMATICITY_SIMPLE)
        else:
            raise ValueError(
                f'unsupported aromaticity {aromaticity}, must be mdl, rdkit, or simple.'
            )
    except ValueError as e:
        traceback.print_exc()
...
def cleanup_rkmol_stereochemistry(rkmol: Chem.Mol,
                                  *,
    ...
    if _rkmol.GetNumConformers() > 0:
        ...
        Chem.AssignStereochemistryFrom3D(_rkmol,
                                         confId=0,
                                         replaceExistingTags=True)
    else:
        Chem.SetBondStereoFromDirections(_rkmol)
        Chem.AssignStereochemistry(_rkmol,
                                   cleanIt=True,
                                   force=True,
