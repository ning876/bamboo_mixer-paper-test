# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/rkutil/resonance.py
...
from .information import (get_nnz_formal_charges,
...
def get_resonance_structures(
        mol: Chem.Mol,
        flags: int = 0,
        filter_by_formal_charges: bool = True) -> List[Chem.Mol]:
    # doc:
...
def get_canonical_resoner(input_mol: Chem.Mol) -> Chem.Mol:
    ...
    cdd_mols = get_resonance_structures(input_mol,
                                        flags=0,
    ...
    if origin_smiles in smiles_mol_map:
        ...
        return copy.deepcopy(
            sorted(list((smi, mol) for smi, mol in smiles_mol_map.items()),
