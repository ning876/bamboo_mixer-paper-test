# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/rkutil/plot.py
...
def plot_molecule_torsion(rkmol: Chem.Mol,
                          torsion_list: Iterable[Iterable],
    ...
    for torsion_atom_indices in torsion_list:
        no_hs_torsion_atom_indices = tuple(
            (no_hs_indices_map[idx] for idx in torsion_atom_indices))
        ...
        img = Draw.MolToImage(mol,
                              size=DEFAULT_PLOT_SHAPE,
                              highlightAtoms=no_hs_torsion_atom_indices)
        img.save('{}_torsion_{}_structure.jpg'.format(
            molecule_name, '_'.join([str(i) for i in torsion_atom_indices])))
    return
...
def show_mol_grid(mols: Iterable[Chem.Mol],
    ...
    for i, mol in enumerate(mols):
        if remove_h:
            ...
            _mol = mol
            _highlight = [] if highlights is None else list(
                highlights[i]).copy()
