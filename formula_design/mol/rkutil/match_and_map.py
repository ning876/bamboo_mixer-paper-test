# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/rkutil/match_and_map.py
...
def find_mapped_smarts_matches(rkmol: Chem.Mol,
    ...
    ** adapted from openff.toolkit.utils.rdkit_wrapper.py **
    ...
    if isinstance(mapped_smarts, str):
        ...
        if qmol is None:
            raise ValueError(
                f'RDKit could not parse the query string "{mapped_smarts}"')
    else:
    ...
    if match_resonance:
        warn(
            'resonance match in rdkit is not reliable, this is only for experimental use',
            UserWarning)
...
def find_indices_mapping_between_isomorphic_mols(
        rkmol0: Chem.Mol,
        rkmol1: Chem.Mol,
        verbose: bool = False,
        ring_matches_ring_only: bool = True,
...
def is_atom_map_num_valid(rkmol: Chem.Mol) -> bool:
    ...
    return sorted([atom_mapnum - 1 for _, atom_mapnum in mapnum
                   ]) == list(range(rkmol.GetNumAtoms()))


def add_atom_map_num(rkmol: Chem.Mol,
                     smarts_atoms: Union[Dict, Iterable[int]] = None) -> None:
    ''' Set atom map number. 
...
def renumber_atoms_with_atom_map_num(rkmol: Chem.Mol) -> Chem.Mol:
    ...
    # map number from 1 to N
    # index from 0 to N-1
    assert sorted([atom_mapnum - 1 for _, atom_mapnum in mapnum
                   ]) == list(range(rkmol.GetNumAtoms()))
    assert sorted([atom_idx for atom_idx, _ in mapnum
                   ]) == list(range(rkmol.GetNumAtoms()))
