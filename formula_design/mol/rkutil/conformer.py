# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/mol/rkutil/conformer.py
...
from rdkit.Geometry.rdGeometry import \
    Point3D  # pylint: disable=no-name-in-module
...
def opt_confs(mol: Chem.Mol,
              *,
              ffoptimizer: str = 'mmff94s',
              n_threads: int = 1,
    ...
    if ffoptimizer not in ffoptimizers:
        raise NotImplementedError(
            f"{ffoptimizer} is not avaliable, choose one from {ffoptimizers}")
...
def generate_confs(rkmol: Chem.Mol,
    ...
    params.useExpTorsionAnglePrefs = kwargs.pop('useExpTorsionAnglePrefs',
                                                False)

    params.useRandomCoords = kwargs.pop('useRandomCoords',
                                        False)  # useful for large molecules
    params.randomSeed = kwargs.pop('randomSeed', 42)
    params.pruneRmsThresh = kwargs.pop('pruneRmsThresh',
                                       -1.0)  # no prune by default
...
def append_conformers_to_mol(mol: Chem.Mol,
                             conformers: List[np.ndarray]) -> Chem.Mol:
