# Some modules used in this code are adapted from: https://github.com/lucidrains/denoising-diffusion-pytorch/blob/main/denoising_diffusion_pytorch/denoising_diffusion_pytorch_1d.py
...
# available at https://github.com/lucidrains/denoising-diffusion-pytorch/blob/main/LICENSE.
#
...
class ResnetBlock(Module):
    ...
    def __init__(self,
        ...
        self.mlp = nn.Sequential(
            nn.SiLU(), nn.Linear(time_emb_dim + context_emb_dim, dim_out * 2)
        ) if exists(time_emb_dim) and exists(context_emb_dim) else None

        self.block1 = Block(dim, dim_out, dropout=dropout)
        self.block2 = Block(dim_out, dim_out)
        self.res_conv = nn.Conv1d(dim, dim_out,
                                  1) if dim != dim_out else nn.Identity()

    def forward(self, x, time_emb=None, context_emb=None):

        scale_shift = None
        if exists(self.mlp) and exists(time_emb) and exists(context_emb):
            all_emb = torch.cat((time_emb, context_emb), dim=-1)
...
class Unet1DSimple(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(2, hidden_dim, kernel_size=3, padding=1), nn.SiLU(),
            nn.Conv1d(hidden_dim, hidden_dim, kernel_size=3, padding=1),
            nn.SiLU())

        self.decoder = nn.Sequential(
            nn.Conv1d(hidden_dim, hidden_dim, kernel_size=3, padding=1),
            nn.SiLU(), nn.Conv1d(hidden_dim, 2, kernel_size=3, padding=1))

    def forward(self, t, x, context):
        """ x: (batch, 1, 384), t: (batch,) """
        x = torch.cat((x, context), dim=1)
        t_emb = self.time_mlp(t).unsqueeze(-1)  # (batch, hidden_dim, 1)
        x = self.encoder(x) + t_emb  # Add time embedding
        x = self.decoder(x)
        return x[:, 0:1, :], x[:, 1:2, :]
...
class Unet1D(Module):
    ...
    def __init__(self,
        ...
        self.cond_mlp = nn.Sequential(nn.Linear(prop_dim, cond_dim), nn.GELU(),
                                      nn.Linear(cond_dim, cond_dim))

        resnet_block = partial(ResnetBlock,
                               time_emb_dim=time_dim,
                               context_emb_dim=cond_dim,
                               dropout=dropout)
        # layers
        ...
        self.mid_attn = Residual(
            PreNorm(
                mid_dim,
                Attention(mid_dim, dim_head=attn_dim_head, heads=attn_heads)))
        ...
        #     nn.AdaptiveAvgPool1d(1),  # (B, 64, 384) -> (B, 64, 1)
        #     nn.Conv1d(init_dim, 1, kernel_size=1),  # (B, 64, 1) -> (B, 1, 1)
        #     nn.Flatten(1)  # (B, 1, 1) -> (B, 1)
        # )
        self.condition_method = condition_method

    def forward(self, time, x, context, x_self_cond=None):

        if self.self_condition:
            x_self_cond = default(x_self_cond, lambda: torch.zeros_like(x))
            x = torch.cat((x_self_cond, x), dim=1)

        # if context is not None and self.condition_method == "concat":
        #     x = torch.cat((x, context), dim=1)
        # elif context is not None and self.condition_method == "add":
        #     x = x + context

        #print("in unet x shape", x.shape)
        x = self.init_conv(x)
        r = x.clone()

        t = self.time_mlp(time)
        c = self.cond_mlp(context)

        #context_emb = self.prop_mlp(context)
        # context_emb = None
...
class GaussianDiffusion1D(Module):
    ...
    def __init__(self,
        ...
        alphas_cumprod_prev = F.pad(alphas_cumprod[:-1], (1, 0), value=1.)  # pylint: disable=not-callable
        ...
        register_buffer('loss_weight', loss_weight)  # pylint: disable=possibly-used-before-assignment
