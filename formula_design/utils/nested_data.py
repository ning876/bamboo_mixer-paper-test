# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/utils/nested_data.py
...
class NestedData:
    ...
    def _check_key(self, __key: DataTypeEnum):
        if isinstance(__key, self.DataTypeEnum):
            ...
            raise TypeError(
                f'Type {type(__key)} is not supported by {type(self)}!')

    def _check_value(self, __key: DataTypeEnum, __value: Union[list,
                                                               torch.Tensor]):
        if isinstance(__value, list):
            __value = torch.tensor(
                __value,
                dtype=self.dtype) if __value else self._default_value(__key)
        else:
            assert isinstance(__value, torch.Tensor)
            if __value.dtype != self.dtype:
                __value = __value.to(self.dtype)
        assert __value.shape[1] == self.width[
            __key], f'{__key}, {__value.shape}'
        return __value
    ...
    def __setitem__(self, __key: DataTypeEnum, __value: Union[list,
                                                              torch.Tensor]):
        self._check_key(__key)
