# The code is adapted from https://github.com/bytedance/byteff/blob/master/byteff/utils/definitions.py
...
SUPPORTED_ELEMENTS = [1, 5, 6, 7, 8, 9, 14, 15, 16, 17,
                      3]  # H, B, C, N, O, F, Si, P, S, Cl, Li

ELEMENT_MAP = {at: i for i, at in enumerate(SUPPORTED_ELEMENTS)}

SOLVENTS = [
    "PC", "DEC", "EC", "EMC", "TFP", "EA", "EFA", "DMC", "MA", "Glyme", "t_BC",
    "2-MeTHF", "FEC", "BUTY", "Sulfolane", "Triglyme", "Tetraglyme", "Diglyme",
    "AN", "TESA", "EM", "PM", "EE", "PE", "THF", "DMM", "DEE", "DEM", "DOL",
    "DMS", "2FEA", "alpha-F-GBL", "beta-F-GBL", "PFPN", "FDMB", "F3DEE",
    "F6DEE", "F4DEE", "F5DEE", "DEG-FTEG", "DME-FTEG", "DEG-FTriG",
    "DME-FTriEG", "TMP", "PhCF3", "MTFA", "FEBFP", "EBFP", "DEBFP", "BFEFP",
    "D2", "DTF", "DFDEC", "DCEP", "TTE", "EMP", "F1EMP", "F2EMP", "F3EMP",
    "EP", "VC", "MF"
]
SOLVENTS_MAP = {sol: i for i, sol in enumerate(SOLVENTS)}

SALTS = [
    "LIPF6", "LIBF4", "LIBOB", "LITFSI", "DCTA", "PDI", "LICF3SO3", "BNB",
    "BBPB", "BBB", "BSB", "LICTFSI", "LITDI", "LICLO4", "LIFSI", "LIDFOB",
    "LIFNFSI"
]
SALTS_MAP = {sol: i for i, sol in enumerate(SALTS)}
...
class TopoParams:
    ...
    def __init__(self,
        ...
        self.counts: dict[TopoTerm, torch.Tensor] = {
            term: None
            for term in TopoTerm
        }
        self.uncertainty: dict[ParamTerm, torch.Tensor] = {
            term: None
